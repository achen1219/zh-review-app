...
function loadDay(date){
  ...
  const dailyChars = getDailyChars(date);

  area.innerHTML = prev!==null
    ? `<div>上次分數：${prev} 分</div><h2>${date} 生字</h2>`
    : `<h2>${date} 生字</h2>`;
  
  dailyChars.forEach(ch => area.appendChild(createFlashcard(ch)));
  ...
  btnQuiz.onclick=()=>startQuiz(date,dailyChars);
...
function createFlashcard(ch){
  const info=tzDict[ch]||{}, 
        ...
        rad=info.radical||'—', 
        def=info.definition||'—', 
        phs=info.phrases||{'2':[], '3':[], '4':[]};
  
  const renderP=(n)=>{ 
    const arr=phs[String(n)]||[]; 
    if(!arr.length) return `<div><strong>常用詞（${n}字）：</strong>—</div>`; 
    return `<div><strong>常用詞（${n}字）：</strong></div><ul>${arr.map(p=>`<li>${p.word} – ${p.zh}</li>`).join('')}</ul>`; 
  };
